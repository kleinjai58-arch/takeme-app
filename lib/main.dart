  ...
  Widget build(BuildContext context){
    return Scaffold(
      ...
      body:Column(children:[
        ...
        Expanded(child:ListView(padding:EdgeInsets.symmetric(horizontal:16),children:rooms[selected]!.map((room)=>Card(color:Color(0xFF1A1A23),shape:RoundedRectangleBorder(borderRadius:BorderRadius.circular(12)),child:ListTile(leading:CircleAvatar(backgroundColor:Color(0xFF8B5CF6),child:Icon(Icons.mic,color:Colors.white,size:18)),title:Text(room,style:TextStyle(fontWeight:FontWeight.w600,fontSize:14)),subtitle:Text("verified • No bots"),trailing:ElevatedButton(onPressed:(){ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text("Joined $room in $selected! 🎉")));},style:ElevatedButton.styleFrom(backgroundColor:Color(0xFF8B5CF6),minimumSize:Size(60,32)),child:Text("Join",style:TextStyle(fontSize:12)))))).toList())),
      ]),
    );
  }
}
